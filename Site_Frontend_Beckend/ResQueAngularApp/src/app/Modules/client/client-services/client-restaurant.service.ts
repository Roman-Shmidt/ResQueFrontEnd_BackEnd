...
export class ClientRestaurantService {
  ...
  createRestaurant(queue: restaurant): Observable<any> {
  ...
  getRestaurantByNumber(number: number): Observable<restaurant> {
    return this.http.get<restaurant>(`${this.apiUrl}/restaurants/${number}`);
  }

  deleteRestaurant(number: number): Observable<any>{
